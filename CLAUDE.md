...
- `UpstreamServiceException` (en `packages/bp-common/src/Clients/`) preserva el `errorCode` de negocio del servicio interno (ej. `step_up_required` de `StepUpAuthMiddleware`), no solo el mensaje y el status code — `HandlesUpstreamErrors::upstreamError()` lo usa en vez de aplanar siempre a `"upstream_error"`. Un frontend que necesite reaccionar a un código de negocio específico (no solo mostrar el mensaje) debe leer `error.code` del envelope, nunca parsear el texto del mensaje (se encontró y corrigió un caso así en `TransferenciaPage.tsx` durante la Fase 10 — llevaba desde la Fase 9 detectando el step-up por substring del mensaje).
