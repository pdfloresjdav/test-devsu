...
**Criterio de aceptación:** todo el código (clases, métodos, rutas HTTP, campos JSON, columnas de base de datos/atributos DynamoDB, nombres de test, comentarios, texto de UI) queda en inglés. Los documentos de gobernanza (`CHECKLIST.md`, `WORKLOG.md`, `CLAUDE.md`, `README.md`, `docs/arquitectura-banca-digital-bp.md`) se quedan en español — no son código. Los nombres de carpeta/servicio (`services/svc-*`, `bff-*`, `frontend-*`) NO se renombran (son infraestructura/despliegue, fuera de alcance — ver plan). Glosario español→inglés y detalle completo del plan en `/Users/pedroflores/.claude/plans/happy-spinning-lampson.md`.
...
- [x] 12.11 Verificación end-to-end completa: wipe de `bp_mysql_data`/`bp_localstack_data` (confirmado con el usuario antes de ejecutar), `make dev` desde cero, repetida la prueba manual de punta a punta de la Fase 11 (login PKCE real vía curl contra mock-oidc, `GET /customers/{id}`, `GET /dashboard/{accountId}`, `POST /accounts/{accountId}/movements`, `POST /transfers` real con débito de saldo confirmado en MySQL, `POST /onboarding` y `POST /revalidate-liveness` de bff-mobile, logs de `svc-auditoria`/`svc-notificaciones` confirmando "Audited:"/"Notified: MovementRegistered/TransferCompleted") con todas las rutas y campos ya en inglés. Bug real encontrado y corregido en el camino: el `Dockerfile` de `svc-movimientos` seguía invocando el comando viejo `movimientos:setup-table` en su `CMD` (no se había verificado con `docker build` real durante 12.3, que solo corrió `php artisan test` en el host) — corregido a `movements:setup-table`; de paso se tradujeron los comentarios en español de los 7 `Dockerfile`. README.md actualizado (rutas, campos de body, nombre de la base `svc_transferencias`→`svc_transfers`, env vars `VITE_DEMO_CUENTA_ID`/`EXPO_PUBLIC_DEMO_CUENTA_ID`→`*_DEMO_ACCOUNT_ID`, comando `movimientos:setup-table`→`movements:setup-table`).
